    ...
    async def _make_decision_async(
        ...
        try:
            ...
            while iteration < max_tool_iterations:
                ...
                print(f"📤 Calling LLM with {len(messages)} messages (analysis stage)...")
                ...
                # Updated to handle optional ** bolding markers commonly output by the LLM
                ...
                if mcp_session and tool_calls:
                    ...
                    # Fallback: If no logic triggered, ensure we append history so we don't loop infinitely on the same prompt
                    print("⚠️  No decision and no tools found. Appending response to history.")
                    messages.append({
                        "role": "assistant",
                        "content": response_text,
                    })
